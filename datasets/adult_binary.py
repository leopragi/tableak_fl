...
import sys
sys.path.append("..")
from utils import to_numeric_binary
import pickle


class ADULT_BINARY(BinaryBaseDataset):

    def __init__(self, name='ADULT_BINARY', single_bit_binary=False, device='cpu', random_state=42):
        super(ADULT_BINARY, self).__init__(name=name, device=device, random_state=random_state)
        ...
        train_data_num = to_numeric_binary(train_data, self.features, label=self.label, single_bit_binary=self.single_bit_binary)
        test_data_num = to_numeric_binary(test_data, self.features, label=self.label, single_bit_binary=self.single_bit_binary)
