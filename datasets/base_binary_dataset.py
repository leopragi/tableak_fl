...
class BinaryBaseDataset(ABC):
    ...
    def decode_batch(self, batch, standardized=True):
        ...
        return to_categorical_binary(batch.clone().detach().cpu(), self.train_features, single_bit_binary=self.single_bit_binary)
    ...
    def encode_batch(self, batch, standardize=True):
        ...
        """
        batch = torch.tensor(to_numeric_binary(batch, self.train_features, label=self.label, single_bit_binary=self.single_bit_binary), device=self.device)
        if standardize:
    ...
    def _create_index_maps(self):
        ...
        for val, key in zip(self.features.values(), self.features.keys()):
            if val is None or (len(val) == 2 and self.single_bit_binary):
                ...
                # CHANGE: PRAGI
                end = int(np.ceil(np.log2(len(val))))
                index_list = [pointer + i for i in range(end)]
                pointer += end
                # CHANGE: PRAGI
